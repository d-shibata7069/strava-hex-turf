...
- **グループの作成:** ヘッダーの「作成」から `/groups/new` に遷移し、グループ名と招待コードを入力して「作成する」を押す。作成者本人が自動的にそのグループのメンバーとして登録される。
...
1. **自分の user ID と所属グループ ID を確認する**
   ...
   - **Table Editor** で `groups` を開く。テスト用グループがなければ「Insert row」で 1 行追加（**name** と **invite_code** を適当に入力）。通常はアプリの `/join` でこの招待コードを入力して参加する。
